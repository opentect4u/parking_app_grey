...
function VehicleInOutStore() {
  ...
  const getVehicleWiseReports = async (fromDate, toDate) => {
   ...
   const getVehicleWiseFixedReports = async (fromDate, toDate) => {
    ...
    console.log(startDate, "------------------Time----------------", endDate)
    return new Promise((resolve, reject) => {
      db.transaction(tx => {
        tx.executeSql(
          //'SELECT a.shift_name,(SELECT COUNT(b.*) FROM vehicleInOutTable AS b WHERE  date(b.date_time_in) >= ? AND date(b.date_time_in) <= ? AND time(b.date_time_in) >= a.f_time AND time(b.date_time_in) <= a.t_time ) AS quantity ,(SELECT SUM(c.paid_amt) FROM vehicleInOutTable AS c WHERE  date(c.date_time_in) >= ? AND date(c.date_time_in) <= ? AND time(c.date_time_in) >= a.f_time AND time(c.date_time_in) <= a.t_time ) as totalAmount, (SELECT SUM(d.advance) FROM vehicleInOutTable AS d WHERE  date(d.date_time_in) >= ? AND date(d.date_time_in) <= ? AND time(d.date_time_in) >= a.f_time AND time(d.date_time_in) <= a.t_time )AS TotalAdvance FROM shiftTable AS a'
          'SELECT vehicleType, COUNT(*) AS quantity, SUM(paid_amt) AS totalAmount, SUM(advance) AS TotalAdvance FROM vehicleInOutTable WHERE date_time_out IS NULL AND  date(date_time_in) >= ? AND date(date_time_in) <= ? GROUP BY vehicleType',
          // 'SELECT vehicleType, COUNT(*) AS quantity, SUM(paid_amt) AS totalAmount, SUM(advance) AS TotalAdvance FROM vehicleInOutTable WHERE date_time_out IS NOT NULL AND date_time_out >= ? AND date_time_out <= ? GROUP BY vehicleType',
         // [startDate.toISOString(), endDate.toISOString(),startDate.toISOString(), endDate.toISOString(),startDate.toISOString(), endDate.toISOString()],
          [startDate.toISOString(), endDate.toISOString()],
          (_, resultSet) => {
  ...
  const getOperatorWiseShiftReports = async (fromDate, toDate) => {
    const db = await getDatabaseConnection();
    ...
    const endDate2 = new Date(toDate);
    const endDateStart = new Date(endDate2.getFullYear(), endDate2.getMonth(), endDate2.getDate());
    const endDate = new Date(endDateStart.getTime() + 24 * 60 * 60 * 1000 - 1);
    ...
    return new Promise((resolve, reject) => {
        db.transaction(tx => {
          ...
          //   SELECT
          //     a.shift_name,
          //     (SELECT COUNT(b.*) FROM vehicleInOutTable AS b WHERE date(b.date_time_in) >= ? AND date(b.date_time_in) <= ? AND time(b.date_time_in) >= a.f_time AND time(b.date_time_in) <= a.t_time) AS quantity,
          //     (SELECT SUM(c.paid_amt) FROM vehicleInOutTable AS c WHERE date(c.date_time_in) >= ? AND date(c.date_time_in) <= ? AND time(c.date_time_in) >= a.f_time AND time(c.date_time_in) <= a.t_time) as totalAmount,
          //     (SELECT SUM(d.advance) FROM vehicleInOutTable AS d WHERE date(d.date_time_in) >= ? AND date(d.date_time_in) <= ? AND time(d.date_time_in) >= a.f_time AND time(d.date_time_in) <= a.t_time) AS TotalAdvance
          //   FROM
          //     shiftTable AS a
          // `,
          `SELECT a.shift_name,
          (SELECT COUNT(b.paid_amt) 
             FROM vehicleInOutTable AS b 
             WHERE date(b.date_time_in) >= ? 
             AND date(b.date_time_in) <= ? 
             AND time(b.date_time_in) >= a.f_time AND time(b.date_time_in) <= a.t_time) AS quantity,
             COALESCE((SELECT SUM(c.paid_amt) 
             FROM vehicleInOutTable AS c 
             WHERE date(c.date_time_in) >= ? 
             AND date(c.date_time_in) <= ? 
             AND time(c.date_time_in) >= a.f_time 
             AND time(c.date_time_in) <= a.t_time),0) as totalAmount,
             COALESCE((SELECT SUM(d.advance) 
             FROM vehicleInOutTable AS d 
             WHERE date(d.date_time_in) >= ? 
             AND date(d.date_time_in) <= ? 
             AND time(d.date_time_in) >= a.f_time 
             AND time(d.date_time_in) <= a.t_time),0) AS TotalAdvance
    FROM shiftTable AS a`,
                [startDate.toISOString(),endDate.toISOString(),startDate.toISOString(),endDate.toISOString(),startDate.toISOString(),endDate.toISOString()],
              // [],
                (_, resultSet) => {
                    const records = [];
  ...
  //   console.log(startDate, "------------------unbilled Time----------------", startDate.toISOString())
  //   return new Promise((resolve, reject) => {
  //     db.transaction(tx => {
  //       tx.executeSql(
  //         'SELECT a.* FROM shiftTable AS a',
  //         [],
  //         // [startDate.toISOString(), endDate.toISOString(),startDate.toISOString(), endDate.toISOString(),startDate.toISOString(), endDate.toISOString()],
  //         (_, resultSet) => {
  ...
  //             records.push(record);
  //           }
  //           console.log("+================+ OPERATORWISE efbrftgbser +================+", records)
  //           resolve(records);
  //         },
  //         (_, error) => {
  //           console.log("+================+ OPERATORWISE RECORDDDD +================", error)
  //           reject(error); // Error occurred during the query
  //         }
  //       );
  //     });
