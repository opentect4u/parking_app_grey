...
let { dev_mod } = generalSetting
  ...
  if(generalSetting?.total_collection!='Y'){
    // Find the index of the object to be removed
    const indexToRemove = todayCollectionArray.findIndex(
      (item) => item.title === 'Total Collection'
    );
    // Remove the object at the found index
    if (indexToRemove !== -1) {
      todayCollectionArray.splice(indexToRemove, 1);
    }
