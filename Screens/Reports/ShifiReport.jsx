...
const ShifiReport = ({ navigation }) => {
    ...
    const { getOperatorWiseShiftReports } = VehicleInOutStore()
    ...
    const handleUnbilledPrint = async () => {
        ...
        const extractedData = unbilledData && unbilledData.map(({ shift_name, quantity,TotalAdvance, totalAmount }) => {
            // opratorName, quantity, TotalAdvance, totalAmount

            // const last4Digits = (vehicle_no.toString()).slice(-4).padStart(4, '0');
            // const paddedResult = last4Digits.padEnd(12, ' ');


            return `${(shift_name?.toString().padEnd(9)).padEnd(12)}${quantity.toString().padStart(4).padEnd(8)}${TotalAdvance.toString().padStart(7).padEnd(14)}${totalAmount.toString().padStart(8)}\n`
            
            // return `${(opratorName?.toString().padEnd(9)).padEnd(15)}${quantity.toString().padStart(4).padEnd(8)}${TotalAdvance.toString().padStart(7).padEnd(14)}${totalAmount.toString().padStart(8)}\n`

        }).join('');

        console.log("object =/=/==/=/=/==/=//==/=", extractedData)
        setpl(true)
        let headerPayload = 'SHIFT WISE REPORT\n'
        ...
        payload += `MC.ID: ${imein} \n`
        payload += "--------------------------------------------------------------------------\n"
        payload += "Shift          Qty       Advance       Amount\n "
        payload += "--------------------------------------------------------------------\n"

        payload += extractedData
        payload += `${(("TOTAL").toString().padEnd(9)).padEnd(11)}${totalQTY.toString().padStart(4).padEnd(8)} ${totalAdvance.toString().padStart(6).padEnd(12)} ${totalPrice.toString().padStart(8)} \n  `
    ...
    async function handleGenerateReport() {
        ...
        setLoading(true)
        getOperatorWiseShiftReports(mydateFrom.toISOString(), mydateTo.toISOString()).then(res => {
            setUnbilledData(res)
    ...
    return (
        <View style={{ flex: 1 }}>
            <CustomHeader title={"Shift Reports"} navigation={navigation} />
